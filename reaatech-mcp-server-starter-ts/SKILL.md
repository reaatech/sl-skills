...
| --- | --- | --- |
| `@reaatech/mcp-server-auth` | published v1.0.0 | Express middleware that validates incoming MCP server requests via API key or Bearer token using constant-time comparison, with an automatic dev-mode bypass when no key is confi… |
| `@reaatech/mcp-server-core` | published v1.0.0 | Provides shared Zod schemas, domain types (`ToolResponse`, `ContentBlock`, `RequestContext`), validated environment configuration, and content block factories for the `@reaatech… |
| `@reaatech/mcp-server-engine` | published v1.0.0 | An Express 5-based MCP server framework that provides a composable middleware pipeline (auth, rate limiting, idempotency, sanitization), dual transport support (Streamable HTTP… |
| `@reaatech/mcp-server-observability` | published v1.0.0 | Provides structured logging (Pino-based with PII redaction), OpenTelemetry tracing with OTLP export, and Prometheus-compatible metrics for MCP servers, exporting a logger, `init… |
| `@reaatech/mcp-server-tools` | published v1.0.0 | A type-safe tool registry and discovery system for MCP servers, providing a `defineTool()` helper with Zod input schemas, an in-memory registry with `registerTool`/`getTools`/`g… |
| `@reaatech/mcp-server-transport` | published v1.0.0 | Mounts Streamable HTTP and SSE MCP transport handlers onto an Express application, managing session lifecycle, automatic cleanup, and transport-level metrics. Exports `mountStre… |
